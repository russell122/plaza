...
var totalWrappers = document.querySelectorAll('.product-element__total-wrapper');
var itemWrappers = document.querySelectorAll('.cart-items__wrapper');
var removeItemButtons = document.querySelectorAll('.button_remove');
var clearCartButton = document.querySelector('.clear-basket-button');
var sliderNext = document.querySelector('.slider__button_next');
var sliderPrev = document.querySelector('.slider__button_prev');

function setTotal(counterValueWrapper, priceWrapper, totalValueWrapper) {
  totalValueWrapper.innerText = parseInt(priceWrapper.innerText) * +counterValueWrapper.innerText + '₽';
}
...
function initSlider() {
  ...
  var viewedSlider = new swiper__WEBPACK_IMPORTED_MODULE_0__["default"]('.swiper-container', {
    loop: true,
    slidesPerView: slides
  });
  sliderNext.addEventListener('click', function () {
    return viewedSlider.slideNext();
  });
  sliderPrev.addEventListener('click', function () {
    return viewedSlider.slidePrev();
  });
}
...
itemWrappers.forEach(function (itemWrapper) {
  itemWrapper.addEventListener('click', function (e) {
    if (e.target.classList.contains('button_remove')) {
      ...
      itemsWrapper.removeChild(e.target.parentElement.parentElement);
    }
  });
});
removeItemButtons.forEach(function (button) {
  button.addEventListener('mouseenter', function () {
    ...
    label.classList.add('remove-label');
    label.innerText = 'Удалить из корзины';
    button.append(label);
  });
  button.addEventListener('mouseleave', function () {
    var label = button.querySelector('.remove-label');
...
totalWrappers.forEach(function (itemWrapper) {
  itemWrapper.addEventListener('click', function (e) {
    console.log(e.target);
    var counterWrapper = itemWrapper.querySelector('.product-total__counter-wrapper');
    ...
    var priceWrapper = itemWrapper.querySelector('.product-element__total-current');
    var totalValueWrapper = itemWrapper.querySelector('.product-total__total-value');
    var counterValue = +counterValueWrapper.innerText;

    if (e.target === counterDecreaseButton) {
      if (counterValue > 0) {
        counterValue--;
        counterValueWrapper.innerText = counterValue;
        setTotal(counterValueWrapper, priceWrapper, totalValueWrapper);
      }
    }

    if (e.target === counterIncreaseButton) {
      counterValue++;
      counterValueWrapper.innerText = counterValue;
      setTotal(counterValueWrapper, priceWrapper, totalValueWrapper);
    }
